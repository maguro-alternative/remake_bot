...
func TestNewLinePostDiscordChannelViewHandler(t *testing.T) {
	...
	t.Run("サーバー一覧が正常に表示される(2件)", func(t *testing.T) {
		indexService := &service.IndexService{
			Client: mock.NewStubHttpClient(func(req *http.Request) *http.Response {
				return &http.Response{
					StatusCode: http.StatusOK,
		...
		assert.Contains(t, rec.Body.String(), `<a href="/guild/234">`)
		assert.Contains(t, rec.Body.String(), `<img src="https://cdn.discordapp.com/icons/234/test2.png" alt="test2">`)
		assert.Contains(t, rec.Body.String(), `<li>test2</li>`)
	})

	t.Run("サーバー一覧が正常に表示される(0件)", func(t *testing.T) {
		indexService := &service.IndexService{
			Client: mock.NewStubHttpClient(func(req *http.Request) *http.Response {
				return &http.Response{
					StatusCode: http.StatusOK,
		...
		assert.NotContains(t, rec.Body.String(), `<a href="/guild/123">`)
		assert.NotContains(t, rec.Body.String(), `<img src="https://cdn.discordapp.com/icons/123/test.png" alt="test">`)
		assert.NotContains(t, rec.Body.String(), `<li>test</li>`)
	})
