...
func NewWebRouter(
	...
	mux.Handle("/login/line", loginRequiredChain.ThenFunc(lineLogin.NewLineLoginHandler(indexService).Index))
	...
	mux.Handle("/guild/{guildId}/permission", discordLoginRequiredChain.ThenFunc(permissionView.NewPermissionViewHandler(indexService).Index))
	mux.Handle("/guild/{guildId}/linetoken", discordLoginRequiredChain.ThenFunc(linetokenView.NewLineTokenViewHandler(indexService).Index))
