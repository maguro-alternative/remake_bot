...
func TestNewLinePostDiscordChannelViewHandler(t *testing.T) {
	...
	t.Run("linetokenが正常に表示されるか", func(t *testing.T) {
	...
	t.Run("linetokenが正常に表示されるか(デフォルトチャンネルが選択済みになるか)", func(t *testing.T) {
		indexService := &service.IndexService{
			DiscordSession: &discordgo.Session{},
		}
		indexService.DiscordBotState = discordgo.NewState()
	...
	t.Run("linetokenが正常に表示されるか(入力済みが0か)", func(t *testing.T) {
		indexService := &service.IndexService{
			DiscordSession: &discordgo.Session{},
		}
		indexService.DiscordBotState = discordgo.NewState()
