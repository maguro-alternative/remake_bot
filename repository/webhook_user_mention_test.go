...
func TestWebhookUserMention(t *testing.T) {
	...
	t.Run("WebhookUserMention取得(idsから取得)", func(t *testing.T) {
		dbV1, cleanup, err := db.NewDB(ctx, config.DatabaseName(), config.DatabaseURLWithSslmode())
		assert.NoError(t, err)
		defer cleanup()
		...
		webhookUserMentions, err := repo.GetWebhookUserMentionWithWebhookSerialIDs(ctx, []int64{*f.Webhooks[0].WebhookSerialID, *f.Webhooks[1].WebhookSerialID})
		assert.NoError(t, err)
		assert.Len(t, webhookUserMentions, 2)
		assert.Equal(t, "111111", webhookUserMentions[0].UserID)
		assert.Equal(t, "111111", webhookUserMentions[1].UserID)
	})
